...
if st.button("Calculate Growth"):
    if start_date >= end_date:
        ...
        with st.spinner('Analyzing market data...'):
            for sym in search_list:
                ...
                if not data.empty and len(data) > 1:
                    ...
                    # LOGIC: Check for Indian Market
                    currency_symbol = "₹" if sym.endswith(".NS") else "$"
        ...
        if results:
            ...
            # Use top performer's currency for the final display
            top_curr = "₹" if top_asset['Asset'].endswith(".NS") else "$"
            
            st.success(f"Best Performer: {top_asset['Asset']}")
            
            final_val = investment_amount * (1 + float(top_asset['Growth (%)'])/100)
            st.metric(f"Your {investment_amount} would become:", f"{top_curr}{final_val:,.2f}", f"{top_asset['Growth (%)']}%")
            ...
            with col_right:
                ...
                if not plot_data.empty:
                    ...
                    fig.update_layout(template="plotly_dark", 
                                      title=f"Price Trend: {top_asset['Asset']} ({top_curr})",
                                      xaxis_title="Date", yaxis_title=f"Price ({top_curr})")
                    st.plotly_chart(fig, use_container_width=True)
        else:
            st.warning("No data found. Try a different range.")